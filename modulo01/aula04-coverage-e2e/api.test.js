...
describe('API Suite Test', () => {
  ...
  describe('/login', () => {
    it(
      'should login successfully on the login route and return HTTP status 200',
      async () => {
        const response = await request(app)
          .post('/login')
          .send({ username: 'ivander', password: '1234' })
          .expect(200);
    ...
    it(
      'should reject login from invalid user/password combination '
      + 'and return HTTP status 401',
      async () => {
        const response = await request(app)
          .post('/login')
